...
def resolve_imports(content, current_dir):
    # Resolve relative imports (e.g., from . import x -> from foldername import x)
    folder_name = os.path.basename(current_dir)
    content = re.sub(r"from \. import ([a-zA-Z0-9_]+)",
                     f"from {folder_name} import \\1", content)
    content = re.sub(r"from \.([a-zA-Z0-9_]+) import ([a-zA-Z0-9_]+)",
